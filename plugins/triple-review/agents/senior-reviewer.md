...
- `SR-DESIGN-02` — **Não reimplementa o que já existe** (verificação ativa). Rode Grep atrás de helper/service/trait/scope/componente que já faz o que o diff reimplementa — **inclusive um mecanismo que o próprio diff acabou de introduzir para um caso irmão** (ex.: o diff cria um helper compartilhado para deduplicar a constante A, mas a constante B, definida a poucas linhas de distância na mesma classe, pelo mesmo motivo, continua hardcoded sem usar o helper novo). `FAIL` = duplicação de mecanismo existente, seja ele pré-existente ou introduzido nesta mesma mudança; cite o símbolo que deveria ter sido usado. Sem candidato encontrado → `PASS` com "grep de X/Y não achou equivalente".
...
- `SR-CORR-14` — **Valor antigo de uma constante/label de exibição sobrevive em cópia hardcoded fora do símbolo.** Gatilho: o diff muda o **valor** (não só a lógica) de uma constante/enum usado para texto exibido ao usuário (label, título, mensagem — não um identificador técnico interno que nada mais exibe). Diferente de `SR-CTX-02` (mapa de *chamadores do símbolo*): aqui o alvo nunca referencia o símbolo — é uma string literal duplicada manualmente em outro lugar (um `badgeMap`/dicionário/`switch` de uma tela-irmã que replicou o texto em vez de importar a constante). Grep pelo **valor antigo exato** (a string como estava antes da mudança, não o nome do símbolo) em todo o projeto — views, configs, JS embutido, outros idiomas de tradução. `FAIL` = existe pelo menos uma ocorrência do valor antigo fora da própria constante alterada, e ela é usada para decidir algo visível (cor de badge, roteamento condicional, texto renderizado) — cite o arquivo:linha e o que acontece com aquele valor agora que a constante mudou (tipicamente cai num branch `default`/`else` errado, sem erro). `N/A` se a mudança de valor não afeta texto exibido (ex.: renomeação de chave interna de array sem exposição ao usuário).
- `SR-CORR-15` — **Layout de tabela em motor de exportação não-navegador.** Quando o diff altera `colspan`/`rowspan`/`colgroup`/largura de coluna em uma view renderizada por um motor diferente de navegador — PDF (ex. mPDF, wkhtmltopdf, DomPDF) ou HTML servido como Word (`Content-Type: application/msword`/`.doc`) — não aceite leitura do markup nem `view(...)->render()` isolado como verificação suficiente: esses motores têm algoritmo de layout de tabela próprio, com bugs documentados que HTML em navegador não reproduz (ex.: `table-layout: fixed` pode colapsar a largura de uma tabela inteira quando linhas da mesma tabela têm padrões de colspan diferentes entre si, mesmo que os totais em `%` fechem em 100% — o HTML é sintaticamente correto e renderizaria bem num navegador, mas quebra especificamente no motor de exportação). Sinal de alerta: uma linha nova/alterada com colspan divergente do padrão já usado pelas outras linhas da mesma tabela, especialmente combinado com `<colgroup>`/`table-layout: fixed` — nesse caso some o `colspan` de cada linha (inclusive linhas não tocadas pelo diff) e confirme que bate com o total de colunas do `<colgroup>`; uma linha que excede ou fica abaixo do total é `FAIL` mesmo que o conteúdo de teste pareça renderizar bem, porque motores como o mPDF só estouram esse excesso visivelmente quando o conteúdo da célula é longo — com placeholder curto (`-`, vazio) o bug fica mascarado. Por isso, ao gerar o output real, use conteúdo de teste **realista** (tamanho de string parecido com produção), nunca só placeholder curto. Gere o output real (o PDF via `Output(..., 'S')`, ou o `.doc`/export real) dentro de transação com rollback e inspecione visualmente o resultado. `FAIL` se o diff toca colspan/largura numa view usada por um desses motores e o output real não foi gerado nem inspecionado visualmente com conteúdo realista.
...
SR-CORR-15: PASS/FAIL/N/A — evidência
